# COPIED FROM: https://raw.githubusercontent.com/pytorch/pytorch/cbcb2b5ad767622cf5ec04263018609bde3c974a/benchmarks/fastrnns/custom_lstms.py
# added "no reverse" version of some functions.
# "reverse" function was causing runtime error in newer pytorch versions.
...
def script_lstm(
    ...
    if bidirectional:
        ...
        if not no_reverse:
            ...
            layer_type = BidirLSTMLayerNoReverse
        dirs = 2
    elif dropout:
...
class ReverseLSTMLayerNoReverse(jit.ScriptModule):
    def __init__(self, cell, *cell_args):
        super(ReverseLSTMLayerNoReverse, self).__init__()
        self.cell = cell(*cell_args)

    @jit.script_method
    def forward(self, input, state):
        # type: (Tensor, Tuple[Tensor, Tensor]) -> Tuple[Tensor, Tuple[Tensor, Tensor]]
        inputs_not_reversed = input.unbind(0)
        outputs = jit.annotate(List[Tensor], [])
        for i in range(len(inputs_not_reversed) - 1, -1, -1):
            out, state = self.cell(inputs_not_reversed[i], state)
            outputs += [out]
        outputs_revered = [outputs[i] for i in range(len(outputs) - 1, -1, -1)]
        return torch.stack(outputs_revered), state
...
class BidirLSTMLayerNoReverse(jit.ScriptModule):
    __constants__ = ["directions"]

    def __init__(self, cell, *cell_args):
        super(BidirLSTMLayerNoReverse, self).__init__()
        self.directions = nn.ModuleList(
            [LSTMLayer(cell, *cell_args), ReverseLSTMLayerNoReverse(cell, *cell_args),]
        )
...
def test_script_stacked_lnlstm_bidi(
    seq_len, batch, input_size, hidden_size, no_reverse=False
):
    inp = torch.randn(seq_len, batch, input_size)
    states = [
        [
    ...
    rnn = script_lnlstm(
        input_size, hidden_size, 1, bidirectional=True, no_reverse=no_reverse
    )

    # states = [
    #     LSTMState(torch.randn(batch, hidden_size), torch.randn(batch, hidden_size)),
    # ]
    # rnn = script_lnlstm(input_size, hidden_size, 1, bidirectional=False)

    # just a smoke test
    out, out_state = rnn(inp, states)

    print(out.shape, out_state[0][0][0].shape)


def test_lnlstm_cell():
    hidden_size = 128
    input_size = 128
    batch = 1
    cell = LayerNormLSTMCell(input_size=input_size, hidden_size=hidden_size)

    states = LSTMState(torch.randn(batch, hidden_size), torch.randn(batch, hidden_size))
    inp = torch.randn(batch, input_size)

    out, out_state = cell.forward(inp, states)

    print(out.shape, out_state[0].shape)
...
if __name__ == "__main__":
    ...
    test_script_stacked_lnlstm_bidi(10, 1, 64, 128, no_reverse=False)
    test_lnlstm_cell()
