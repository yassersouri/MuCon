...
class Viterbi(object):
    ...
    def set_multi_length(self, mode=True):
        pass
    ...
    def decode(self, log_frame_probs):
        ...
        for t in range(
            2 * self.frame_sampling - 1, frame_scores.shape[0], self.frame_sampling
        ):
    ...
    def init_decoding(self, frame_scores):
        ...
        for label in self.grammar.possible_successors(context):
            ...
            score = self.grammar.score(context, label) + self.frame_score(
                frame_scores, self.frame_sampling - 1, label
            )
        ...
        for key, hyp in old_hyp.items():
            ...
            if length + self.frame_sampling <= self.length_model.max_length():
                ...
                new_hyp.update(
                    new_key,
                    score,
                    self.TracebackNode(label, hyp.traceback, boundary=False),
                )
            ...
            for new_label in self.grammar.possible_successors(context):
                ...
                score = (
                    hyp.score
                    + self.frame_score(frame_scores, t, label)
                    + self.length_model.score(length, label)
                    + self.grammar.score(context, new_label)
                )
                new_hyp.update(
                    new_key,
                    score,
                    self.TracebackNode(new_label, hyp.traceback, boundary=True),
                )
    ...
    def traceback(self, hyp, n_frames):
        ...
        labels += [hyp.traceback.label] * (
            n_frames - len(labels)
        )  # append labels for missing frames
